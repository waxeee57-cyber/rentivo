...
import {
  ...
  step, section, finish, day,
} from './_lib.mjs'
import { FIXTURES, assertFixture } from './fixtures.mjs'

/**
 * The listing this suite owns, under an operator row whose auth_id is the
 * e2e-operator account below. The seeded operator's auth_id belongs to the
 * project owner, who must not be signed in as, so the operator side of the chat
 * needs an identity this script actually controls.
 *
 * It used to share listing e2ec0000-…cafe and the +150..+190 window with
 * contract.mjs. Two suites booking the same vehicle in the same window race for
 * nights and eventually exhaust it, and the loser reports "no free window" —
 * which looks like a product bug and is not one.
 */
const FX = FIXTURES.messaging
 ...
 *
 * This used to pick a random start in +150..+188 and retry ten times, because
 * the window was shared and re-runs piled bookings onto the same nights. With
 * the window owned and cleared at startup the offsets can be fixed, so a failure
 * here is a real failure rather than "somebody else got there first".
 */
async function bookAt(token, offset, label) {
  ...
  if (res.status === 200 && id) return { id, start: offset, res }
...
// Fail loudly on the wrong vehicle rather than quietly opening a thread on it.
const fixture = await assertFixture(sb, 'messaging', traveler.token)
step(true, 'messaging fixture is ours', `${fixture.row.title}, +${FX.from}..+${FX.to}`)
...
const booked = await bookAt(traveler.token, FX.from + 4, 'main')
...
const raceBooking = await bookAt(traveler.token, FX.from + 10, 'race')
...
section('7 — Cleanup: release the nights this run held')

// The main booking is paid, so it holds its dates until it is cancelled. The
// conversation and the messages are not touched by the cancellation — the thread
// belongs to the booking row, not to the availability block — so the proof above
// stays readable while the next run gets its dates back.
...
step(leftLive.list.length === 0, 'no booking of ours is still holding dates in the messaging window', `n=${leftLive.list.length}`)
const threadSurvives = await rows(traveler.token, `rentivo_messages?conversation_id=eq.${convId}&select=id`)
step(threadSurvives.list.length >= 2, 'and the chat thread survived the cleanup', `n=${threadSurvives.list.length}`)
